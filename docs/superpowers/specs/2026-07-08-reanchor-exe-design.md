...
1. **同电脑换目录**：全自动零交互。`directory` 重写为新位置；工具链 `-I` 仍存在，原样保留。
2. **换电脑**：工具链 `-I` 失效 → 触发 Keil 探测（`KEIL_PATH` env → `~/.keil2clangd.json` → 常见位置扫描 → **都没有才交互询问**，答案存入 config，下次不问）→ 自动重写失效 `-I` 与 `directory`。标准安装位置时全程无交互。
   - pack 版本不同（如旧机 CMSIS 5.4.0 新机只有 5.6.0）：前缀替换后路径仍不存在 → 该行保留原样 + 警告，提示跑 skill 重新生成（手术工具不猜版本）。
3. 双击运行结束前「按回车退出」，保证摘要可读。
